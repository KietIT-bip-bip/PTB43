...
while so_bai_kiemtra <= 0:
    _bai_kiemtra = int(input(" Nhap lai so bai kiem tra > 0"))

for i in range(so_bai_kiemtra):
    diem = float(input(f"Nhap diem { i + 1}:"))
    while diem <0 :
        diem = float(input("Nhap lai diem hop le:"))
    # them danh sach
...
if (so_bai_kiemtra == 1): print("Ko the xoa dp danh sach 1 phan tu:")
else:
